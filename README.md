...
## v36 — Multiusuário real, tenant, Postgres (Fase 1 da integração BuyerRadar)

A partir desta versão o sistema abandonou a senha única compartilhada. Setup:

```bash
# 1. Configure DATABASE_URL (Postgres em produção; sqlite:///./buscador_processos.db serve pra dev local)
# 2. Rode as migrations
alembic upgrade head

# 3. Crie o primeiro administrador (nunca existe cadastro público)
python cli.py bootstrap-admin --email voce@meuprecatoriobr.com.br --full-name "Seu Nome" --password "sua-senha"

# 4. A partir daí, administradores criam os demais usuários via POST /api/admin/users
```

Papéis disponíveis: `administrador`, `pesquisador`, `juridico`, `comercial`, `aprovador`, `visualizador` — cada um com permissões efetivas (ver `app/models_auth.py::PERMISSOES_POR_PAPEL`), não só um nome guardado no banco.

Um tenant (`meuprecatoriobr`, por padrão) já cobre o uso atual; a estrutura está pronta para múltiplos tenants no futuro sem precisar refazer o schema.

Migração de dados do BuyerRadar (organizations/funds/people/contacts), sempre em modo `--dry-run` por padrão:
```bash
python cli.py migrate-buyerradar --source "postgresql://.../buyerradar" --dry-run
```

**Pendências explícitas para a Fase 2+** (não implementadas nesta rodada): motor de matching/scoring de compradores, geração de teaser, disparo de campanha (estrutura de dados pronta, envio real atrás de `CAMPAIGN_SENDING_ENABLED=false` por padrão), robôs de descoberta de compradores (CVM/institucional).
